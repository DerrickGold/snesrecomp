"""Stable generated-bank translation-unit splitting."""
import os
import sys

THIS = os.path.dirname(os.path.abspath(__file__))
REPO = os.path.abspath(os.path.join(THIS, os.pardir, os.pardir))
TOOLS = os.path.join(REPO, 'tools')
RECOMPILER = os.path.join(REPO, 'recompiler')
for path in (TOOLS, RECOMPILER):
    if path not in sys.path:
        sys.path.insert(0, path)

import v2_regen  # noqa: E402


SOURCE = '''/* generated */
#include "cpu_state.h"

/* Forward declarations for in-bank entries. */
RecompReturn bank_00_8000_M1X1(CpuState *cpu);
RecompReturn bank_00_8800_M1X1(CpuState *cpu);

RecompReturn bank_00_8000_M1X1(CpuState *cpu) {
  return bank_00_8800_M1X1(cpu);
}

RecompReturn bank_00_8800_M1X1(CpuState *cpu) {
  return RECOMP_RETURN_NORMAL;
}

void ResetHandler(CpuState *cpu) {
  RecompReturn r = bank_00_8000_M1X1(cpu);
}
'''

ENTRY_STARTS = {
    'bank_00_8000': 0x8000,
    'bank_00_8800': 0x8800,
    'ResetHandler': 0x8000,
}


def test_large_bank_splits_by_stable_pc_range():
    parts = v2_regen._split_bank_translation_units(
        SOURCE, 0, ENTRY_STARTS, threshold_bytes=0, pc_span=0x800)
    assert set(parts) == {'bank00_part00_v2.c', 'bank00_part01_v2.c'}
    assert 'bank_00_8000_M1X1(CpuState *cpu) {' in parts['bank00_part00_v2.c']
    assert 'void ResetHandler(CpuState *cpu) {' in parts['bank00_part00_v2.c']
    assert 'bank_00_8800_M1X1(CpuState *cpu) {' in parts['bank00_part01_v2.c']
    assert 'bank_00_8000_M1X1(CpuState *cpu) {' not in parts['bank00_part01_v2.c']


def test_chunk_embeds_only_referenced_variant_declarations():
    parts = v2_regen._split_bank_translation_units(
        SOURCE, 0, ENTRY_STARTS, threshold_bytes=0, pc_span=0x800)
    part0 = parts['bank00_part00_v2.c']
    assert 'RecompReturn bank_00_8800_M1X1(CpuState *cpu);' in part0
    part1_preamble = parts['bank00_part01_v2.c'].split(
        'RecompReturn bank_00_8800_M1X1(CpuState *cpu) {', 1)[0]
    assert 'bank_00_8000_M1X1(CpuState *cpu);' not in part1_preamble


def test_small_bank_keeps_monolithic_filename_and_bytes():
    parts = v2_regen._split_bank_translation_units(
        SOURCE, 0, ENTRY_STARTS, threshold_bytes=len(SOURCE) + 1,
        pc_span=0x800)
    assert parts == {'bank00_v2.c': SOURCE}
